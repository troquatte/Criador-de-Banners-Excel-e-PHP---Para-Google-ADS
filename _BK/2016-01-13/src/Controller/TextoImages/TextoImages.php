...
{	
	...
	private function ConfigJsonLayout()
	{
		...
		$arquivoConfig	= getcwd() . "/controller/TextoImages/configTexto.json";
	...
	private function ConfigJsonTexto()
	{
		...
		$arquivoConfig	= getcwd() . "/controller/TextoImages/config.json";
		//Copia Todo o conteúdo do json em uma string
		$informacaoJson = file_get_contents($arquivoConfig);
				...
				foreach ($value as $linha => $coluna) {
					...
					if($linha !== 0){
						...
						foreach ($config as $tamanhoImagens => $value) {
							...
							$caminhoPastaImagem = getcwd() . "/../images/" . $tamanhoImagens . ".png";
								...
								if(file_exists($caminhoPastaImagem)){
									...
									$font = getcwd() . "/.." . $configCaminhoPastaFont["fonte"];
									...
									imagepng($imagem,  getcwd() . "/../download/" . $destino . "/" . $destino . "-" . $tamanhoImagens . ".png");
								}
								// Final If Exsists
						}
						//Final foreach
